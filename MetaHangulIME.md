...
한글 조합의 확정 단위와 삭제 동작을 정의합니다.

#### 음절 단위 관리
- **특징**: 음절 단위로 조합 관리 및 삭제
- **백스페이스**: 도깨비불 현상 이후에도 음절 단위 삭제
- **예시**: 
  ```
  갃 + ㅣ → 각시
  백스페이스 → 각ㅅ (시 전체 삭제)
...
#### 명시적 커밋이 있는 입력기
- **특징**: 명시적 커밋 전까지 낱자 단위로 세밀한 조합 관리
- **백스페이스**: 역 도깨비불 현상 지원
- **명시적 커밋 시점**: 
  - 비-한글 글자 입력 (영문, 특수문자, 띄어쓰기, 엔터 등)
  - 커서 이동
  - 입력 중단 후 일정 시간(보통 수 초) 경과
- **예시**: 
  ```
  갃 + ㅣ → 각시
  백스페이스 → 갃 (ㅣ만 삭제, 역 도깨비불)
  ```
...
### 3. 커밋 단위 테스트
- 음절 단위: 갃ㅣ → 각시 → (BS) → 각ㅅ → (BS) → 각 → (BS) → (빈 상태)
- 명시적 커밋: 갃ㅣ → 각시 → (BS) → 갃 → (BS) → 각 → (BS) → 가
